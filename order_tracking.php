    ...
    function addTracking($conn, $oid, $status, $loc, $desc){
        try {
            ...
            if($is_pdo){
                ...
                $s=$conn->prepare("INSERT INTO tracking_logs (order_id,status,location,description) VALUES (?,?,?,?)");
...
try {
    if($is_pdo){
        ...
        $conn->exec("ALTER TABLE orders ADD COLUMN IF NOT EXISTS courier VARCHAR(50) DEFAULT 'BUKID2BAYAN Xpress'");
...
<style>
.track-wrap{ max-width:1120px; margin:20px auto 20px auto; padding:0 16px; display:grid; grid-template-columns:1fr 420px; gap:16px; }
@media(max-width:900px){.track-wrap{ grid-template-columns:1fr; margin-top:10px; } }
</style>
  ...
  <div>
    <div style="background:#fff; border:1px solid #e9e9e9; border-radius:14px; padding:16px; margin-bottom:14px;">
        <div style="display:flex; justify-content:space-between; flex-wrap:wrap; gap:8px;">
            ...
            <span style="font-size:0.85rem; color:#555;"><?= htmlspecialchars($order['courier']??'BUKID2BAYAN Xpress')?> • <?= htmlspecialchars($order['payment_method']??'COD')?> • ₱<?= number_format($order['total_amount']??0,2)?></span><br>
        ...
        <div style="margin-top:14px; background:#f5f5f5; height:8px; border-radius:10px; overflow:hidden;"><div style="width:<?= $percent?>%; height:100%; background:#111;"></div></div>
